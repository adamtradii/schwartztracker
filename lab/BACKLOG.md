...
- [ ] **Stocks mean-reversion at 1-min**: the momentum study found −0.045 autocorrelation at 1 min on real data. Test a short-horizon reversion strategy (e.g. fade N-sigma 1-min moves, tight exits) on stocks-real across all 10 windows. Does it clear 6 bps round-trip costs?
- [ ] **Bar resampling**: add 5-min/15-min resampling to ReplayAdapter (aggregate 1-min bars), re-run momentum study and SMA configs on slower bars. Does trend edge appear at slower timescales net of costs?
